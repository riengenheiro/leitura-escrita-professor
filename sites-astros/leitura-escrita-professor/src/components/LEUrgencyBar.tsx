...
function MiniCountdown() {
  ...
  useEffect(() => { setTimeLeft(getTimeLeft()) }, [])
  useEffect(() => {
...
function PurchaseNotification() {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 max-w-xs" style={{ animation: 'slideIn 0.4s ease-out' }}>
      ...
      <div className="bg-white border-2 border-btn-primary/40 rounded-xl p-4 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-gray-900 text-sm font-medium truncate">{notification.name} acabou de garantir</p>
            <p className="text-gray-500 text-xs">{notification.city} • {notification.time}</p>
          </div>
        </div>
...
export function LEUrgencyBar({ price }: LEUrgencyBarProps) {
  ...
  useEffect(() => { setActivity(getActivityData()) }, [])
  useEffect(() => {
  ...
  const messages = [
    ...
    { icon: TrendingUp, text: `${activity.purchasesToday} professoras garantiram nas últimas 24h`, color: 'text-btn-primary' },
    { icon: Flame, text: `Oferta de R$ ${pagePrice} por tempo limitado`, color: 'text-red-500' },
  ]
        ...
        >
          ...
          <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <span className="text-sm text-white truncate">{messages[currentMessage].text}</span>
            </div>
            <div className="hidden sm:flex items-center gap-3">
              ...
              <a href="#opcoes" className="px-4 py-1.5 bg-accent hover:opacity-90 text-gray-900 text-sm font-bold rounded-lg transition-all">
                GARANTIR AGORA
              </a>
            </div>
