...
export function FMCountdown() {
  ...
  useEffect(() => {
    ...
    if (endTime && endTime > now) setSecondsLeft(endTime - now);
    else {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      setSecondsLeft(Math.max(0, endTime - Math.floor(Date.now() / 1000)));
    }, 1000);
  ...
  return (
    <div className="bg-gray-900 text-white py-3 px-4 text-center sticky top-0 z-50 shadow-lg animate-countdown">
      <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-center gap-2 md:gap-4">
        ...
        <span className="text-accent font-bold text-sm md:text-base">Oferta termina em:</span>
        <span className="font-black text-xl md:text-2xl tabular-nums text-white min-w-[4rem]">{display}</span>
      </div>
    </div>
  );
}
